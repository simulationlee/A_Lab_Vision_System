...
# import trained model (YOLO5-based model for Nut detection)
model = torch.hub.load('ultralytics/yolov5', 'custom', path='nut_detection/weights/best.pt', source='local')
model.conf = 0.5
...
def dectshow(org_img, boxs,depth_data):
    ...
    for box in boxs:
        ...
        cv2.putText(img, box[-1] + ' ' + str(dist / 1000)[:4] + 'm',
                    (int(box[0]), int(box[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
